...
class UsersTable{
    ...
    public function getAll(){
        ...
        SELECT users.*,role.name AS role,role.value FROM users LEFT JOIN role ON users.role_id = role.id
        ");
        return $statement->fetchAll();
    }

    public function findByEmailAndPassword($email,$password){
        $statement = $this->db->prepare("SELECT users.*,role.name As role,role.value FROM users LEFT JOIN role ON users.role_id = role.id WHERE users.email = :email AND users.password = :password");
